...
export const encrypt = async (msg: Buffer, password: string): Promise<EncryptedData> => {
    const sparams = {
        ...{
            salt: randomBytes(32),
            iv: randomBytes(16)
    ...
    const mac = keccak256(
        bufferToHex(Buffer.concat([Buffer.from(derivedKey.slice(16, 32)), Buffer.from(ciphertext)]))
    )
    return {
...
export const decrypt = async (encryptedData: EncryptedData, password: string): Promise<Buffer> => {
    const sparams = {
        ...{
