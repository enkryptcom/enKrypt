<template>
  <div class="container" :class="{ popup: isPopup }">
    ...
    <div v-if="showTxHash && txHashRef" class="tx-hash-success">
      <p>
        Transaction sent!<br />Transaction hash:
        <span class="tx-hash">{{ txHashRef }}</span>
      </p>
...
import { AnimicaTransactionSigner } from '../../libs/signer';
import AnimicaAPI from '../../../libs/api';
import { AnimicaNetwork } from '../../../networks/animica-base';
import { TX_VALIDITY_WINDOW } from '../../../libs/transaction';
...
const txHashRef = ref<string | null>(null);
...
const network = ref<BaseNetwork>(DEFAULT_ANIMICA_NETWORK);
...
const sendAction = async () => {
  ...
  try {
    const animicaNetwork = network.value as AnimicaNetwork;
    ...
    // Transfers are valid for a window of blocks starting at the current head
    const head = await api.getHead();

    // Sign the transaction
    const signedTx = await AnimicaTransactionSigner({
      account: fromAccount,
      ...
      payload: {
        ...
        validAfter: head.height,
        validUntil: head.height + TX_VALIDITY_WINDOW,
      },
    });

    // Pre-flight through the node's admission path, then broadcast
    const txHash = await api.broadcast(signedTx.rawTx);
    txHashRef.value = txHash;
    ...
    const txActivity: Activity = {
      ...
      chainId: animicaNetwork.chainId.toString(),
