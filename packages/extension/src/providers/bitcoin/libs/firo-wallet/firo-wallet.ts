...
export class FiroWallet implements AbstractWallet {
  ...
  async getSpendableUtxos(numAddresses: string[]) {
    ...
    console.log(`🔍 Checking up to ${numAddresses.length} derived addresses...`);
    ...
    for (let index = 0; index < numAddresses.length; index++) {
      ...
      const { address: derivedAddress } = bitcoin.payments.p2pkh({ pubkey: child.publicKey, network: this.network });

      if (address !== derivedAddress) {
        console.error(`🚨 Derived address mismatch! Expected: ${address}, Got: ${derivedAddress}`);
      } 
      ...
      try {
          ...
          console.error(`❌ Error fetching UTXOs for ${address}:`, (error as {message: string}).message);
      }
    }
  ...
  private async fixDuplicateCoinIssue(): Promise<boolean> {
    ...
    unspentCoins.forEach(coin => {
      const duplicateCoins = this._lelantus_coins_list.filter(c => c.index == coin.index);
