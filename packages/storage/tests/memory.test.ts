...
describe('Simple addition', () => { // the tests container
    ...
    it('it should properly handle storage operations', async () => {
        ...
        expect(await memStore.getWholeStorage()).to.deep.equal({ "test-namespace1": { "newObj": { abc: 1, efg: 2 } } })
        await storage2.set("newObj", { abc: 3, efg: 4 })
        expect(await memStore.getWholeStorage()).to.deep.equal({ "test-namespace1": { "newObj": { abc: 1, efg: 2 } }, "test-namespace2": { "newObj": { abc: 3, efg: 4 } } })
        ...
        expect(await memStore.getWholeStorage()).to.deep.equal({ "test-namespace2": { "newObj": { abc: 3, efg: 4 } } })
    });
});