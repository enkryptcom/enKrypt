...
describe('Polkadot Address generate', () => { // the tests container
    ...
    it('sr25519 addresses should work', async () => {
        ...
        let keypair = await signer.generate(MNEMONIC, "//0")
        expect(keypair.address).equals("5FqSRmrJGMEdb5xk58k6x9RUw7Ri9oEKckDApupjMDysDrZL");
        keypair = await signer.generate(MNEMONIC, "//1")
        expect(keypair.address).equals("5F2NJSfSZ1ZtUiHXKc3XLKY6SwU3Y1jVw4CD2mKW4okBHNnx");
        keypair = await signer.generate(MNEMONIC, "//0//0")
        expect(keypair.address).equals("5GKwdnFRMxHP8hTjPY39eF88oCzoUQURawFpx2sJhCMCP15D");
        const list = [
            {
        ...
        list.forEach(async (item) => {
            keypair = await signer.generate(PHRASE, item.uri)
